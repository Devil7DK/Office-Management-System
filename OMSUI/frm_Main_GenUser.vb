...
Public Class frm_Main_GenUser
    ...
    End Sub
    Sub ReLoadJob()
        dgv_Jobs.DataSource = GlobalCode.DatabaseIO.LoadJob(ConnectionString)
    End Sub
    Sub ReLoadClients()
        Dim cr As New System.ComponentModel.BindingList(Of CredentialWithClient)
        dgv_Clients.DataSource = LoadClients(ConnectionString, Jobs, cr)
        dgv_Credentials.DataSource = cr
    End Sub
    Sub ReLoadUsers()
        dgv_Users.DataSource = LoadUsers(ConnectionString)
    End Sub
    Sub ReLoadWorks()
        dgv_WorkBook.DataSource = LoadWorks(ConnectionString, Clients, Jobs, Users)
    End Sub
    Sub ReLoadHome()
    ...
    Private Sub frm_Main_GenUser_FormClosing(sender As Object, e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        My.Settings.Save()
        Try
            If TcpClientEx1.TCPClient.Connected Then
                TcpClientEx1.Disconnect()
            End If
    ...
    End Sub
    Sub LoadSettings()
        Me.txt_ServerName.Text = My.Settings.Server
        Me.txt_ServerIP.Text = My.Settings.ServerIP
        Me.txt_ServerPort.Text = My.Settings.Port
        Me.txt_SQLServerPassword.Text = My.Settings.Password
        Me.txt_SQLServerUserID.Text = My.Settings.UserID
        Me.txt_DatabaseName.Text = My.Settings.Database
        Me.txt_DefaultStorage.Text = My.Settings.DefaultStorage
        Me.txt_Pooling.IsOn = My.Settings.Pooling
    End Sub
    Sub SaveSettings()
        My.Settings.Server = Me.txt_ServerName.Text
        My.Settings.ServerIP = Me.txt_ServerIP.Text
        My.Settings.Port = Me.txt_ServerPort.Text
        My.Settings.Password = Me.txt_SQLServerPassword.Text
        My.Settings.UserID = Me.txt_SQLServerUserID.Text
        My.Settings.Database = Me.txt_DatabaseName.Text
        My.Settings.DefaultStorage = Me.txt_DefaultStorage.Text
        My.Settings.Pooling = Me.txt_Pooling.IsOn
        My.Settings.Save()
    End Sub
    Private Sub MainForm_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        ...
        Dim t As New Threading.Thread(Sub()
                                          TcpClientEx1.Connect(My.Settings.ServerIP, My.Settings.Port)
                                      End Sub)
        t.Start()
    ...
    Private Sub TcpClientEx1_CommunicationReceived(sender As Object, e As GlobalCode.CommunicatingObject) Handles TcpClientEx1.CommunicationReceived
        If e.MessageTo = "ALL" Or e.MessageTo = UserData.Username Then
